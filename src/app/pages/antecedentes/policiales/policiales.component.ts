...
export default class PolicialesComponent {
  ...
  getAPolicialNumDoc() {
    ...
    this.columns.set([
      "Tip.Doc", "Tip.Doc", "N°Doc", "Nombres", "Ap.Paterno",
      "Ap.Materno", "Cod.Persona",
      "Doble Identidad", "Nacimiento", "Homonimia",
      "Padre", "Madre", "Sexo", "Talla", "Contextura"
    ])

    if (this.docIdentidad().length > 7) {
      this.pideService.getAPolicialNumDoc(post).subscribe({
        next: (res) => {
          const data = res.consultarPersonaNroDocResponse.RespuestaPersona;
  ...
  getAPolicialNomPatMat() {
    ...
    this.pideService.getAPolicialNomPatMat(post).subscribe({
      next: (res) => {
        ...
        const message = dataRaw.descripcionMensaje;
        const data = Array.isArray(dataRaw) ? dataRaw : [dataRaw];

        if (dataRaw.codigoMensaje == 16) {
          this.sweetAlertService.error('Opps!', message)
          return;
        }
